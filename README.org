...
Location data for mileage logs may eventually be recorded in a `mileage_location` relationship using Android's [[https://developer.android.com/reference/android/location/LocationManager][~LocationManager~]]; receipt uploads suffice for now, removing the need for permission to access device's fine location for now.
